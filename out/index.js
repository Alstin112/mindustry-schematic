...
exports.Schematic = exports.Renderer = exports.SchematicAddons = exports.Fluid = exports.Item = exports.UnknownBlock = exports.DefaultBlock = void 0;
const canvas_1 = require("canvas");
const zlib_1 = require("zlib");
const helpers_1 = require("./helpers");
class DefaultBlock {
    constructor(name, options = {}) {
        this.AdditionalImageLayers = [];
        this.requirements = [];
    ...
    get building() {
        return this.constructor.building;
    }
}
exports.DefaultBlock = DefaultBlock;
DefaultBlock.autoprefix = null;
// set imageData ohno loaded from
DefaultBlock.ohno = new canvas_1.Image();
(() => {
...
DefaultBlock.building = class DefaultBlockBuilding {
    constructor(block, schematic, { x, y, rotation }) {
        this.block = block;
    ...
    getBuildingAtRotation(direction = this.rotation) {
        switch (direction) {
            case helpers_1.Direction.right: return this.getBuildingRight();
            case helpers_1.Direction.down: return this.getBuildingDown();
            case helpers_1.Direction.left: return this.getBuildingLeft();
            case helpers_1.Direction.up: return this.getBuildingUp();
        }
        return null;
    }
    extends(block) {
    ...
    drawImage(ctx, offsetX = 0, offsetY = 0) {
        var _a;
        ...
        ctx.rotate(((_a = this.rotation) !== null && _a !== void 0 ? _a : 0) * (Math.PI / 2));
        ctx.translate(-(offsetX + 16), -(offsetY + 16));
        ctx.drawImage(image, offsetX, offsetY);
        ctx.restore();
    }
    drawOverlay(ctx, offsetX = 0, offsetY = 0) { }
    getConfigBuffer() {
        return [0, null];
    }
    requestImage(...names) {
...
UnknownBlock.building = class unknownBlockBuilding extends DefaultBlock.building {
...
exports.Fluid = Fluid;
class SchematicAddons {
    constructor(options) {
        this.addonName = "Vanilla";
        this.ImageRequesters = [];
        this.ItemColorRequesters = [];
        this.FluidColorRequesters = [];
        this.TeamColorRequesters = [];
        this.BlockMap = {};
        this.Items = new Map();
        this.Fluids = new Map();
        if (!options)
            return;
        if (options.ImageRequester)
            this.ImageRequesters.push(options.ImageRequester);
        if (options.ItemColorRequester)
            this.ItemColorRequesters.push(options.ItemColorRequester);
        if (options.TeamColorRequester)
            this.TeamColorRequesters.push(options.TeamColorRequester);
        if (options.FluidColorRequester)
            this.FluidColorRequesters.push(options.FluidColorRequester);
        if (options.BlockMap)
            this.BlockMap = options.BlockMap;
        if (options.Items)
            this.Items = options.Items;
        if (options.Fluids)
            this.Fluids = options.Fluids;
    }
    add(addons) {
...
exports.SchematicAddons = SchematicAddons;
class Renderer {
    constructor() {
        this.time = 0;
        this.bridgeOpacity = 0.7;
    }
}
...
class Schematic {
    constructor() {
        ...
        this.addon = new SchematicAddons();
        this.unknownBlocksNames = [];
    ...
    setFrom(schematic) {
        ...
        if (type !== "msch")
            throw new Error("Invalid input, not a Mindustry Schematic");
        const version = bSchema.readUInt8(4);
        if (version !== 1)
            throw new Error("Invalid version, expected 1 but got " + version);
        const inflatedBuffer = (0, zlib_1.inflateSync)(bSchema.subarray(5));
        let index = 0;
    ...
    static ReadObject(buffer, index) {
    ...
    toBuffer() {
        ...
        for (let i = 0; i < this.buildings.length; i++) {
            ...
            let block = this.blocks.indexOf(building.block);
            if (block === -1)
                throw new Error(`Block '${building.block}' not found in blocks list`);
    ...
    get buildingIdArray() {
        if (this._tileIdArray)
            return this._tileIdArray;
    ...
    // #endregion
    toImageBuffer() {
        const canvas = (0, canvas_1.createCanvas)((this.width + 2) * 32, (this.height + 2) * 32);
